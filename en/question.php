...
<body>
    ...
    if(isset($q_date)){
        ...
        <p><?= $q_princ;?></p>
        <hr>
        <small>Publish by<?= $q_author.' the '. $q_date?></small>
        </section>
        ...
        <section class="show-answer">
            ...
            <?php if(isset($_SESSION['id']) && $_SESSION['pseudo'] !== null ) : ?>
                <span>Answer to <?= $q_title;?></span>
            <textarea name="answer" placeholder="Answer to <?= $q_author ?>"></textarea>
            <input type="submit" name="posta"value="Répondre à <?= $q_author ?>">
  <?php else : ?>
    <br>
    <span>If you want to answer you'll need to connect !</span>
  <?php endif; ?>
